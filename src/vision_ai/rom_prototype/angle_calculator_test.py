...
def _draw_label(canvas: np.ndarray, text: str, pt: tuple[int, int]) -> None:
    """
    노란 글씨 + 반투명 검은 배경 박스로 각도 레이블을 그립니다.
    텍스트가 캔버스 밖으로 나가지 않도록 자동 클리핑합니다.
    """
    (tw, th), _ = cv2.getTextSize(text, FONT, FONT_SCALE, FONT_THICKNESS)
    x, y = pt

    # 텍스트 박스가 캔버스를 벗어나지 않도록 보정
    ...
    cv2.rectangle(
        canvas,
        (x - LABEL_PADDING, y - th - LABEL_PADDING),
        (x + tw + LABEL_PADDING, y + LABEL_PADDING),
        (30, 30, 30), cv2.FILLED
    )
    # 노란 텍스트
...
def build_angle_image(
    ...
    for joint_result in angle_report.joints:
        ...
        # 관절 위치에서 살짝 위로 오프셋
        lx, ly = coords[idx]
        _draw_label(canvas, label_text, (lx + 8, ly - 10))
