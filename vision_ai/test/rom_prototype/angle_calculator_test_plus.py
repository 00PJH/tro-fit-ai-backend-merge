...
# ── 절대 경로 기반 프로젝트 루트 추가 ─────────────────────────────────────────────────────
# 이 파일 위치: vision_ai/test/rom_prototype/angle_calculator_test_plus.py
_THIS_DIR     = Path(__file__).resolve().parent
_PROJECT_ROOT = (_THIS_DIR / ".." / ".." / "..").resolve()  # trofit/
if str(_PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(_PROJECT_ROOT))

from vision_ai.test.media_pipe_test.landmarks import (
