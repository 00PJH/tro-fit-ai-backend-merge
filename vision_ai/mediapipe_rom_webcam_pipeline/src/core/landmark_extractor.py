...
def draw_landmarks_on_frame(
    ...
    for pose_landmarks in detection_result.pose_landmarks:
        ...
        # 3. 관절 포인트 (흰색 외곽 + 좌/우 색상 내부원) — 반지름 2배 확대 (가독성 개선)
        for idx, pt in coords.items():
            cv2.circle(annotated, pt, 6, (255, 255, 255), -1, lineType=cv2.LINE_AA)  # 흰색 외곽원 (r=6)
            ...
            cv2.circle(annotated, pt, 4, inner_color, -1, lineType=cv2.LINE_AA)  # 컬러 내부원 (r=4)
